...
from ultralytics.nn.tasks import DETECT_CLASS, V10_DETECT_CLASS, DetectionModel, SegmentationModel, WorldModel
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        model.float()
        model = model.fuse()
        export_detect_modules = DETECT_CLASS + V10_DETECT_CLASS + (Detect, RTDETRDecoder)
        for m in model.modules():
            if isinstance(m, export_detect_modules):  # includes custom detect heads in extra_modules
