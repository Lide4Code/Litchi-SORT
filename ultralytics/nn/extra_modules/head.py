...
from ..modules.conv import autopad, DWConv
from .block import *
...
class Detect_SEAM(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        #    nn.Sequential(DWConv(c3, c3, 3), SEAM(c3, c3, 1, 16)),
        #    nn.Conv2d(c3, self.nc, 1)
        #) for x in ch)

        self.stem = nn.ModuleList(nn.Sequential(
            nn.Sequential(DWConv(x, x, 3), Conv(x, x, 1)),
            nn.Sequential(DWConv(x, x, 3), SEAM(x, x, 1, 16)),
        ) for x in ch)
        ...
        #    nn.Sequential(DWConv(c2, c2, 3), SEAM(c2, c2, 1, 16)),
...
####################################################################################################
class Detect_SEAMv10(nn.Module):
    """YOLOv8 Detect head for detection models."""
...
class v10Detect_SEAM(Detect_SEAMv10):
    """
