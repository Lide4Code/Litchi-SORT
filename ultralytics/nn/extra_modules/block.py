...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C2f_AdditiveBlock', 'C2f_AdditiveBlock_CGLU', 'CSP_MSCB', 'EUCB', 'C2f_MSMHSA_CGLU', 'CSP_PMSFA', 'CSP_FPMSFA', 'C3k2_DWR', 'C3k2_DWR_DRB']
...
class C3k2_DWR(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            #C3_DWR(self.c, self.c, 2, shortcut, g, e) if c3k else DWR(c_) for _ in range(n)
            C3_DWR(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
class C3k2_DWR_DRB(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.ModuleList(
            #C3_DWR_DRB(self.c, self.c, 2, shortcut, g, e) if c3k else DWR_DRB(c_) for _ in range(n)
            C3_DWR_DRB(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
######################################## Faster Partial Multi-Scale Feature Aggregation Block start ########################################
class MLP(nn.Module):
    def __init__(self, in_dim, hidden_dim):
        super().__init__()
        self.fc1 = nn.Conv2d(in_dim, hidden_dim, 1)
        self.fc2 = nn.Conv2d(hidden_dim, in_dim, 1)

    def forward(self, x):
        return self.fc2(torch.relu(self.fc1(x)))

class FPMSFA(nn.Module):
    def __init__(self, inc) -> None:
        super().__init__()

        self.conv1 = Partial_conv3(inc, n_div=4)
        self.conv2 = Partial_conv3(inc // 2, n_div=inc // 2)
        self.conv3 = Partial_conv3(inc // 4, n_div=inc // 4)
        self.conv4 = Conv(inc, inc, 1)

        #self.mlp = MLP(inc, int(inc * 2))
...
class CSP_FPMSFA(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)

        self.m = nn.ModuleList(FPMSFA(self.c) for _ in range(n))

######################################## Faster Partial Multi-Scale Feature Aggregation Block end ########################################